...
int can_txmsg(CAN_TXINFO *txinfo){
		...
		mailbox->CAN_MB_HD1 = CAN_MB_HD1_CODE(0x1000)
							| CAN_MB_HD1_IDE(0)
							| CAN_MB_HD1_RTR(0)
		...
		mailbox->CAN_MB_HD1 |= CAN_MB_HD1_CODE(0x1100);//active tx buffer
		return 0;
}
/******************************************************************************
* Function: send_CAN_message(uint32_t id, uint8_t data)
* Description: send can message by MB0 with 1 data byte
******************************************************************************/
...
int can_rxbuff_init(int buff_num, int id){
	CAN_MB * mailbox = (CAN_MB *)&(CAN0->RAMn[MSG_BUF_SIZE * buff_num]); /* CAN msg buffer */
	...
	mailbox->CAN_MB_HD1 |= CAN_MB_HD1_CODE(0x0100); /* activate RX buffer */
